...
def create_app(orchestrator, github_webhook_secret: str = "") -> Quart:
    ...
    async def clear_team_data(team_id):
        ...
        _ALLOWED_TABLES = frozenset({"events", "alerts", "profiles", "system_logs",
    ...
    async def replay_team(team_id):
        ...
        if source in ("git", "all"):
            ...
            if git_path and git_path.exists():
                git_events, _merged, _authors = load_git_events(str(git_path))
    ...
    async def database_status():
        ...
        _DB_STATUS_TABLES = frozenset({"events", "alerts", "profiles", "logs"})
        result = {}
        for table in _DB_STATUS_TABLES:
            try:
    ...
    @app.route("/api/teams/<team_id>/collaboration")
    async def team_collaboration(team_id):
        """Serve cached collaboration metrics for a team."""
        if team_id not in orchestrator.teams:
            return jsonify({"error": "not found"}), 404
        if not orchestrator._store:
            return jsonify({"error": "no store"}), 500

        snapshots = await orchestrator._store.read_collaboration_snapshots(team_id)
        latest = snapshots[-1] if snapshots else None
        return jsonify({
            "team_id": team_id,
            "latest": latest,
            "history": snapshots,
            "sprint_count": len(snapshots),
        })

    @app.route("/api/teams/<team_id>/collaboration/compute", methods=["POST"])
    async def compute_collaboration(team_id):
        """Trigger collaboration metric computation for a team."""
        runner = orchestrator.teams.get(team_id)
        if not runner:
            return jsonify({"error": "not found"}), 404

        timeline = list(runner.bus.timeline)
        members = set(runner.config.member_map.values())
        pm_canonical = runner.config.member_map.get(runner.config.pm_user_id, "")
        ...
        if orchestrator._store:
            await orchestrator._store.write_collaboration_snapshot(team_id, 0, metrics)

        return jsonify({"status": "computed", "metrics": metrics})

    @app.route("/api/teams/overview")
    async def teams_overview():
        """Cross-team collaboration summary for zero-click identification."""
        if not orchestrator._store:
            return jsonify({"error": "no store"}), 500

        teams = []
        for tid, runner in orchestrator.teams.items():
            latest = await orchestrator._store.read_latest_collaboration(tid)
            entry = {
                "team_id": tid,
                "team_name": runner.config.team_name,
            }
            if latest:
                entry.update({
                    "health_score": latest.get("health_score"),
                    "gini": latest.get("gini"),
                    "bus_factor": latest.get("bus_factor"),
                    "clustering_ratio": latest.get("clustering_ratio"),
                })
            teams.append(entry)

        # Sort worst-first by health score
        teams.sort(key=lambda t: t.get("health_score") or 999)
