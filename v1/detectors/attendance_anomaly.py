    ...
    async def detect(self, event: Event) -> list[Alert]:
        ...
        if event.action == "session.present":
            ...
            if not self._has_activity_near(activity, event.timestamp, self.activity_window):
                # Only flag if we have substantial activity data (not early in replay)
                total_activity_events = sum(len(v) for v in s["member_activity"].values())
                if total_activity_events > 20:
