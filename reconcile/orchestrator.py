...
from .analyze.commit_classifier import (
    CommitClassifier, InferenceEngine, NLI_AVAILABLE,
)
...
class TeamRunner:
    ...
    def __init__(self, config: TeamConfig, mode: str = "live", store: Store | None = None):
        ...
        self._analyzer = HistoricalAnalyzer(
            pipeline_map=config.pipeline_map,
            email_dir=config.email_dir or "statusreports/",
        )
    ...
    def _wire(self):
        ...
        # Inject team-level config into detector kwargs where constructor accepts them
        # Resolve pm_user_id to canonical name so detectors can compare against event.actor
        pm_canonical = cfg.member_map.get(cfg.pm_user_id, cfg.pm_user_id)
        team_injectables = {"pm_user_id": pm_canonical}
        for name, cls in available.items():
            ...
            # Also inject team-level config values if the constructor accepts them
            for k, v in team_injectables.items():
        ...
        if self.mode == "live" and cfg.ws_url:
            # Build join payload from env if credentials available
            join_payload = None
            userid = os.environ.get("PMTOOL_USERID", "")
            apikey = os.environ.get("PMTOOL_APIKEY", "")
            if cfg.board_id and userid and apikey:
                join_payload = {
                    "action": "join",
...
class Orchestrator:
    ...
    def __init__(self, mode: str = "live", db_path: str = "data/reconcile.db"):
        ...
        # NLI commit classifier (shared across all teams)
        self._inference_engine: InferenceEngine | None = None
        self._commit_classifier: CommitClassifier | None = None
    ...
    async def run(self) -> None:
        ...
        # Initialize NLI engine (fail-fast: errors surface here, not on first event)
        if NLI_AVAILABLE:
            self._inference_engine = InferenceEngine()
            await self._inference_engine.initialize()
            self._commit_classifier = CommitClassifier(engine=self._inference_engine)
            log.info("NLI commit classifier initialized")
        else:
            self._commit_classifier = CommitClassifier(engine=None)
            log.info("torch not available — commit classification using keyword + diff only")
        ...
        try:
            ...
            if self._inference_engine:
                await self._inference_engine.shutdown()
    ...
    @property
    def commit_classifier(self) -> CommitClassifier | None:
        return self._commit_classifier
