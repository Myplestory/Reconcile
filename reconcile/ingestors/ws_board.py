...
class BoardWSIngestor(BaseIngestor):
    ...
    def _extract_metadata(self, action: str, msg: dict) -> dict:
        ...
        # Extract card title for cold path classification (NLI on card titles)
        card_name = msg.get("card_name", "") or msg.get("title", "")
        if card_name:
    ...
    async def stream(self) -> None:
        """Connect and stream events. Auto-reconnect with exponential backoff + jitter."""
        try:
            import websockets
        except ImportError:
        ...
        while True:
            try:
                ...
                async with websockets.connect(
                    ...
                    close_timeout=10,
                    ping_interval=20,
                    ping_timeout=self.read_timeout,
                ) as ws:
                    log.info("Connected to board WebSocket (%s)", self.source_name)
                    attempts = 0  # reset on successful connect

                    # Send join payload (board subscription + greeting)
                    if self.join_payload:
                        await ws.send(json.dumps(self.join_payload))
                        log.info("reconcile says hi o/")
